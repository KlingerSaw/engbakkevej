-- Insert bylaws data if table is empty
INSERT INTO bylaws (section_number, title, content)
SELECT d.section_number, d.title, d.content
FROM (VALUES 
...
) AS d(section_number, title, content)
WHERE NOT EXISTS (SELECT 1 FROM bylaws);